...
#|
     ...
     Assignment 2: Santorini Player with Cards

     This is a Santorini game player agent which plays
     against other player agents.

     Do to time constraints, the majority of this assignment was
     more about implementation in terms of a functional language
     rather than having the best strategy. Therefore, this agent
     makes purely random moves when playing and isn't the best.
     But it does do really well against other random agents :)
|#
...
; Check if the passed in board represents a winning move
...
(define (can-move? board token-pos)
  (> (length (get-valid-move-spaces board token-pos)) 0))

(define (can-move-with-card? board card token-pos original-pos)
  (case card
    [(Apollo) (> (length (apollo board token-pos)) 0)]
    [(Artemis) (> (length (artemis board token-pos original-pos)) 0)]
    [(Atlas) (> (length (get-valid-move-spaces board token-pos)) 0)]
    [(Demeter) (> (length (get-valid-move-spaces board token-pos)) 0)]
    [(Hephastus) (> (length (get-valid-move-spaces board token-pos)) 0)]
    [(Minotaur) (> (length (minotaur board token-pos)) 0)]
    [(Pan) (> (length (get-valid-move-spaces board token-pos)) 0)]
    [(Prometheus) (> (length (get-valid-move-spaces board token-pos)) 0)]))
...
(define (perform-action-of card original-board selected-token valid-move-spaces selected-move-space moved-board valid-build-spaces selected-build-space build-board)
  (case card
    [(Apollo) (let* ([valid-move-spaces (apollo original-board selected-token)]
                     ...
                     [moved-board (apollo-swap (move-player-token original-board selected-token selected-move-space) selected-token selected-move-space)]
                ...
                (if (winning-move? moved-board)
                    moved-board
                    build-board))]
    [(Artemis) (if (can-move-with-card? moved-board 'Artemis selected-move-space selected-token)
                   (let* ([valid-move-spaces2 (artemis moved-board selected-move-space selected-token)]
                          [selected-move-space2 (list-ref valid-move-spaces2 (random 0 (length valid-move-spaces2)))]
                          [moved-board2 (move-player-token moved-board selected-move-space selected-move-space2)]
                     ...
                     (if (winning-move? moved-board) ; If the first move was already a winning move, don't move again
                         moved-board
                         (if (winning-move? moved-board2)
                             moved-board2
                             build-board)))
                   build-board)]
    [(Atlas) (let* ([build-board (atlas moved-board selected-build-space)])
               build-board)]
    [(Demeter) (let* ([valid-build-spaces2 (demeter build-board selected-move-space selected-build-space)])
                 (if (> (length valid-build-spaces2) 0)
                     (let* ([selected-build-space2 (list-ref valid-build-spaces2 (random 0 (length valid-build-spaces2)))]
                            [build-board2 (build-at-space moved-board selected-build-space2)])
                       build-board2)
                     build-board))]
    [(Hephastus) (let* ([build-board2 (hephastus build-board selected-build-space)])
                   build-board2)]
    [(Minotaur) (let* ([opponent-tokens (player-tokens (list-ref (board-players original-board) 1))]
                       [valid-move-spaces (minotaur original-board selected-token)]
                       [selected-move-space (list-ref valid-move-spaces (random 0 (length valid-move-spaces)))]
                       [moved-board (let* ([minotaur-move (move-player-token-minotaur original-board selected-token selected-move-space)])
                                      (if (equal? minotaur-move #f) ; If we end up in an opponent's space and they can't be pushed, revert to a regular move
                                          (move-player-token original-board selected-token selected-move-space)
                                          minotaur-move))]
                       [valid-build-spaces (get-valid-build-spaces moved-board selected-move-space)]
                       [selected-build-space (list-ref valid-build-spaces (random 0 (length valid-build-spaces)))]
                       [build-board (build-at-space moved-board selected-build-space)])
    ...
    [(Pan) (let* ([valid-pan-spaces (pan original-board selected-token)])
             (if (> (length valid-pan-spaces) 0)
                 (move-player-token original-board selected-token (first valid-pan-spaces))
                 (let* ([valid-build-spaces (get-valid-build-spaces moved-board selected-move-space)]
                        [selected-build-space (list-ref valid-build-spaces (random 0 (length valid-build-spaces)))]
                        [build-board (build-at-space moved-board selected-build-space)])
                   (if (winning-move? moved-board)
                       moved-board
                       build-board))))]
    [(Prometheus) (let* ([prometheus-valid-build-spaces (get-valid-build-spaces original-board selected-token)]
                         [prometheus-selected-build-space (list-ref prometheus-valid-build-spaces (random 0 (length prometheus-valid-build-spaces)))]
                         [prometheus-build-board (build-at-space original-board prometheus-selected-build-space)]
                         [valid-move-spaces (prometheus prometheus-build-board selected-token)]
                         [selected-move-space (if (= (length valid-move-spaces) 0)
                                                  selected-move-space
                                                  (list-ref valid-move-spaces (random 0 (length valid-move-spaces))))]
                         [moved-board (if (= (length valid-move-spaces) 0)
                                          (move-player-token original-board selected-token selected-move-space)
                                          (move-player-token prometheus-build-board selected-token selected-move-space))]
...
(define (moveV1 board)
  (let* ([player (list-ref (board-players board) 0)]
         ...
         [selected-token  (let* ([tok (list-ref (player-tokens player) (random 0 2))])
                            (if (can-move? board tok)
                                tok
                                (if (equal? tok (first (player-tokens player)))
                                    (second (player-tokens player))
                                    (first (player-tokens player)))))]
    ...
    (cond
      ...
      [else (if (equal? card 'Pan)
                (increment-turn card-board)
                (if (= (random 0 2) 0)
